...
"""
    struct GaussianHjmModelVolatilityStatic
...
This variant uses static arrays.
"""
struct GaussianHjmModelVolatilityStatic
    HHfInv::SMatrix
    sigma_f::BackwardFlatVolatility
...
function value(
    o::GaussianHjmModelVolatilityStatic,
    u::ModelTime,
    )
    #
    σ_f = o.sigma_f(u)
    σ_f = SVector{length(σ_f)}(σ_f)
    return o.HHfInv * (o.DfT .* σ_f)  # beware DfT multiplication
end


"Calculate volatility matrix."
(o::GaussianHjmModelVolatilityStatic)(u::ModelTime) = value(o, u)


"""
    struct GaussianHjmModelStatic <: SeparableHjmModel
        alias::String
...
struct GaussianHjmModelStatic <: SeparableHjmModel
    ...
    delta::ParameterTermstructure
    chi::ParameterTermstructure
    sigma_T::GaussianHjmModelVolatilityStatic
    y::AbstractArray
    state_alias::AbstractVector
    factor_alias::AbstractVector
    correlation_holder::Union{CorrelationHolder, Nothing}
...
Create a Gausian HJM model with static arrays.
"""
function gaussian_hjm_model_static(
    alias::String,
    delta::ParameterTermstructure,
    chi::ParameterTermstructure,
    sigma_f::BackwardFlatVolatility,
    ...
    HHfInv = SMatrix{d,d}(HHfInv)
    sigma_T = GaussianHjmModelVolatilityStatic(HHfInv, sigma_f, DfT)
    ...
    return GaussianHjmModelStatic(alias, delta, chi, sigma_T, y,
        state_alias, factor_alias, correlation_holder, quanto_model)
end

"""
    parameter_grid(m::GaussianHjmModelStatic)
...
"""
function parameter_grid(m::GaussianHjmModelStatic)
    return m.sigma_T.sigma_f.times
end

"""
    correlation_holder(m::GaussianHjmModelStatic)

Return the model's `CorrelationHolder`.
"""
function correlation_holder(m::GaussianHjmModelStatic)
    return m.correlation_holder
end

"""
    state_dependent_Theta(m::GaussianHjmModelStatic)
...
"""
    state_alias_H(m::GaussianHjmModelStatic)

Return a list of state alias strings required for (H * X) calculation.
"""
state_alias_H(m::GaussianHjmModelStatic) = state_alias(m)

"""
    state_dependent_H(m::GaussianHjmModelStatic)

Return whether H requires a state vector input X.
"""
state_dependent_H(m::GaussianHjmModelStatic) = false

"""
    factor_alias_Sigma(m::GaussianHjmModelStatic)

Return a list of factor alias strings required for (Sigma(u)^T Gamma Sigma(u)) calculation.
"""
factor_alias_Sigma(m::GaussianHjmModelStatic) = factor_alias(m)

"""
    state_dependent_Sigma(m::GaussianHjmModelStatic)

Return whether Sigma requires a state vector input X.
"""
state_dependent_Sigma(m::GaussianHjmModelStatic) = false


### specialised HJM model functions
...
"""
function chi_hjm(m::GaussianHjmModelStatic)
    χ = m.chi()
    return SVector{length(χ)}(χ)
end
...
"""
function benchmark_times(m::GaussianHjmModelStatic)
    δ = m.delta()
    return SVector{length(δ)}(δ)
end

"""
    func_y(m::GaussianHjmModelStatic, t::ModelTime)

Calculate variance/auxiliary state variable y(t).
"""
function func_y(m::GaussianHjmModelStatic, t::ModelTime)
    d = length(m.delta())
    ...
    σ = m.sigma_T((t0+t)/2)
    σ = SMatrix{d,d}(σ)
    return func_y(y0, χ, σ, t0, t)
end


"""
    Theta(
        m::GaussianHjmModelStatic,
        s::ModelTime,
        t::ModelTime,
...
"""
    log_bank_account(m::GaussianHjmModelStatic, model_alias::String, t::ModelTime, X::ModelState)

Retrieve the integral over sum of state variables s(t) from interest rate model.
"""
function log_bank_account(m::GaussianHjmModelStatic, model_alias::String, t::ModelTime, X::ModelState)
    @assert alias(m) == model_alias
    return X(state_alias(m)[end])
end

"""
    log_zero_bond(m::GaussianHjmModelStatic, model_alias::String, t::ModelTime, T::ModelTime, X::ModelState)

Calculate the zero bond term [G(t,T)' x(t) + 0.5 G(t,T)' y(t) G(t,T)]' from rates model.
"""
function log_zero_bond(m::GaussianHjmModelStatic, model_alias::String, t::ModelTime, T::ModelTime, X::ModelState)
    @assert alias(m) == model_alias
...
"""
    log_zero_bonds(m::GaussianHjmModelStatic, model_alias::String, t::ModelTime, T::AbstractVector, X::ModelState)

Calculate the zero bond terms [G(t,T)' x(t) + 0.5 G(t,T)' y(t) G(t,T)]' from rates model.
"""
function log_zero_bonds(m::GaussianHjmModelStatic, model_alias::String, t::ModelTime, T::AbstractVector, X::ModelState)
    @assert alias(m) == model_alias
    idx = X.idx[state_alias(m)[1]]
    d = length(state_alias(m)) - 1  # exclude s-variable
    G = G_hjm(m, t, T)

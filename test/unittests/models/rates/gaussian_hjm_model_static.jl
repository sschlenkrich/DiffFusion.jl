...
@testset "GaussianHjmModelStatic methods." begin
    ...
    @testset "Model setup." begin
        m = DiffFusion.gaussian_hjm_model_static("Std",delta,chi,sigma_f,nothing,nothing)
        ...
        #
        @test_throws AssertionError DiffFusion.gaussian_hjm_model_static("Std", DiffFusion.flat_parameter("", delta()[2:end]), chi, sigma_f, nothing, nothing)
        @test_throws AssertionError DiffFusion.gaussian_hjm_model_static("Std", delta, DiffFusion.flat_parameter("", chi()[2:end]), sigma_f, nothing, nothing)
        @test_throws AssertionError DiffFusion.gaussian_hjm_model_static("Std", DiffFusion.flat_parameter("", reverse(delta())), chi, sigma_f, nothing, nothing)
        @test_throws AssertionError DiffFusion.gaussian_hjm_model_static("Std", delta, DiffFusion.flat_parameter("", reverse(chi())), sigma_f, nothing, nothing)
    end
    ...
    @testset "Variance calculation." begin
        ...
        @test y[1] == y_ref[:,:,1]
        @test isapprox(y[2], y_ref[:,:,2], atol=4.0e-10)
        ...
        m = DiffFusion.gaussian_hjm_model_static("1F", DiffFusion.flat_parameter("Std", [ 0.0 ]), DiffFusion.flat_parameter("Std", [ 0.10]), sigma_f_1, nothing, nothing)
    ...
    @testset "Theta calculation." begin
        m = DiffFusion.gaussian_hjm_model_static("Theta_3F",delta,chi,sigma_f,nothing,nothing)
    ...
    @testset "Model Correlation." begin
        ...
        m_wo_corr = DiffFusion.gaussian_hjm_model_static("Std",delta,chi,sigma_f, nothing, nothing)  
        m_w_corr = DiffFusion.gaussian_hjm_model_static("Std",delta,chi,sigma_f, ch, nothing)
    ...
    @testset "Covariance calculation." begin
        ...
        DiffFusion.set_correlation!(ch, "Std_f_2", "Std_f_3", 0.80)
        DiffFusion.set_correlation!(ch, "Std_f_1", "Std_f_3", 0.50)
        m = DiffFusion.gaussian_hjm_model_static("Std",delta,chi,sigma_f,ch,nothing)
    ...
    @testset "Access model state variables" begin
        ...
        #
        @test typeof(y) == SMatrix{3, 3, Float64, 9}
        @test typeof(G) == SVector{3, Float64}
        @test typeof(df1) == MVector{3, Float64}
        @test typeof(df2) == MVector{3, Float64}
        @test typeof(cmp) == MVector{3, Float64}
        #
        dfT = DiffFusion.log_zero_bonds(m, DiffFusion.alias(m), 4.0, (@SVector [4.0, 6.0, 8.0, 10.0]), SX)
        @test size(dfT) == (3, 4)
